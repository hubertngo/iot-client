...
const MATH_EXPRESSION_DELIMETER = '$';
...
const punctuationMatch = (idx, text) => {
	...
	if (punctuationIdx >= 0 && spaceMatch(text[idx + 1])) {
		// a math expressions could ends with `} $`
		if (!text[idx + 2]) {
			return true;
		} else if (text[idx + 2] !== MATH_EXPRESSION_DELIMETER) {
			return true;
		}
		return false;
	}
...
const mathDelimiter = (idx, text) => text[idx] === MATH_EXPRESSION_DELIMETER;

const checkMatch = (idx, text, max, min) => {
	if (idx < max && idx > min && (punctuationMatch(idx, text) || mathDelimiter(idx, text))) {
		return true;
	}
};
...
const trimText = (text, min = 80, ideal = 100, max = 200) => {
	// This main function uses two pointers to move out from the ideal, to find the first instance of a punctuation mark followed by a space.
